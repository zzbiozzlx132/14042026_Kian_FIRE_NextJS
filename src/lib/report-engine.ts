...
export function dateRange(type: "daily" | "weekly" | "monthly" | "quarterly" | "yearly") {
  const now = new Date();
  ...
  if (type === "daily") {
    ...
    return {
      current: { from: today, to: new Date(today.getTime() + 86_400_000) },
      prev: { from: yesterday, to: today },
      label: "Hôm nay",
      prevLabel: "hôm qua",
    };
  }
  if (type === "weekly") {
    ...
    return {
      current: { from: weekStart, to: new Date(weekStart.getTime() + 7 * 86_400_000) },
      prev: { from: prevWeekStart, to: weekStart },
      label: "Tuần này",
  ...
  if (type === "monthly") {
    ...
    return {
      current: { from: monthStart, to: new Date(now.getFullYear(), now.getMonth() + 1, 1) },
      prev: { from: prevMonthStart, to: monthStart },
      label: "Tháng này",
  ...
  if (type === "quarterly") {
    ...
    return {
      current: { from: qStart, to: new Date(now.getFullYear(), q * 3 + 3, 1) },
      prev: { from: prevQStart, to: qStart },
  ...
  return {
    ...
    prev: { from: prevYearStart, to: yearStart },
    label: `Năm ${now.getFullYear()}`,
    prevLabel: `năm ${now.getFullYear() - 1}`,
  };
}
